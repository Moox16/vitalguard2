...
async function checkForNewAlerts() {
  for (const p of patients) {
    ...
    if (!ev) continue;
    if (ev.alert_state === 'RED_ALERT') {
      await createAlert(p.id, 'red', 'Sem resposta após queda').catch(() => {});
    } else if (ev.alert_state === 'GREEN_ALERT') {
      await createAlert(p.id, 'green', 'Movimento súbito detectado').catch(() => {});
    }
  }
}
